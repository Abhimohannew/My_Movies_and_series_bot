...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAADBQADhwIAAnXeti1cdcRjdVh_BwI')
            return
        if not btn:
            return
...
⭕ Not in my Database""",
           ...
           await asyncio.sleep(5) await client.send_message.delete() 
        )  
            return
        if not btn:
            return
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            if int(index) == int(data["total"]) - 2:
                ...
                buttons.append(
                    [InlineKeyboardButton("👈 𝗕𝗮𝗰𝗸", callback_data=f"back_{int(index)+1}_{keyword}"),InlineKeyboardButton("𝗡𝗲𝘅𝘁 👉", callback_data=f"next_{int(index)+1}_{keyword}")]
                )
                buttons.append(
                    [InlineKeyboardButton(f"📃 Pages {int(index)+2}/{data['total']}", callback_data="pages")]
                )
...
<b>○ Creator :</b> <a href="https://t.me/ff_new_movies_group">My Dev🇹</a>
